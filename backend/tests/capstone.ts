...
describe("capstone", () => {
  ...
  it("Creates a project successfully", async () => {
    ...
    const projectName = "MyTestProject1";
    ...
    assert.strictEqual(
      projectAccount.projectAuthority.toString(),
      user.publicKey.toString()
    );

    assert.strictEqual(projectAccount.projectName, projectName);
    assert.strictEqual(
      projectAccount.targetAmount.toString(),
    ...
    assert.strictEqual(projectAccount.projectState.funding !== undefined, true);
    ...
    assert.strictEqual(projectAccount.bump, projectBump);
